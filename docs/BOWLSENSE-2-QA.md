...
| Sites package contract | Pass | D1 migrations are stored under root `drizzle/`, staged into `dist/.openai/drizzle/` during the build, asserted by CI, and checked against the worker's runtime schema. `0001_tournament_active.sql` preserves archive state for existing databases. |
