...
[Sites manages the sign-in flow](https://learn.chatgpt.com/docs/sites) and supplies `oai-authenticated-user-email`; BowlSense does not accept a client-provided login token. The deployment depends on the Sites runtime stripping any inbound client value for this header and overwriting it only after platform authentication. If that platform guarantee changes, private routes must remain disabled until an equivalent signed identity mechanism is in place. A shell that loads while private API calls return `401` usually means the project allowlist or Sites-authenticated identity is missing or mismatched.
